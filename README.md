...
- **グラフ表示**
    ...
    - **DNA・トレンド連動**: DNA分析で得られた加重平均（PER/PBR/ROE/利回り/増配年数）やテクニカル評価（モメンタム）、集中リスク（HHI）をリアルタイムにグラフへ反映。
- **保有銘柄一覧**
...
## 設定のカスタマイズ (highlight_rules.json)

本アプリの各種判定基準（スコア加点、色のハイライト、購入注目フラグ、レーダーチャートの基準値）は、ルートディレクトリの `highlight_rules.json` を編集することで自由に変更可能です。

### 主要な設定項目

#### 1. ファンダメンタルズ評価 (per, pbr, yield, roe)
- **undervalued**: この値以下（利回りとROEは以上）の場合、テーブルが緑色にハイライトされ、総合評価スコアが加算されます。
- **overvalued**: この値以上の場合、テーブルが赤色にハイライトされます。

#### 2. 連続増配 (consecutive_increase)
- **good / excellent**: 連続増配バッジの色とスコア加点の基準となる年数です。

#### 3. テクニカル指標 (trend)
- **short_term_days / medium_term_days**: 移動平均線の期間設定（デフォルト: 25日/75日）。
- **fibonacci**: 押し目買いのターゲットとなる戻り率の範囲（50.0%〜78.6%など）。
- **rci**: 底値圏を判定するRCI(26)の閾値（-80以下など）。

#### 4. 購入注目フラグ (buy_signal)
- **fundamental_min**: 「注目」フラグを出すために最低限必要なファンダメンタルズスコア。
- **fundamental_diamond**: 「ダイヤモンド💎」を付与するためのファンダメンタルズスコア閾値。
- **rsi_oversold / rci_bottom**: 「売られすぎ」を判定するテクニカル指標の境界値。

#### 5. レーダーチャートのベンチマーク (radar_chart)
- **benchmarks**: チャート上に点線で表示される「市場平均」または「自分の目標値」です。DNA分析結果と比較して強み・弱みを判断する基準になります。
- **labels**: 各軸の名称を変更できます。

#### 6. システム設定 (system)
- **update_cooldown_seconds**: 「全件更新」ボタンのクールダウン時間（秒）です。

### 編集のヒント
- **より厳格に判定したい場合**: `per.undervalued` を `10.0` に下げる、`roe.undervalued` を `15.0` に上げるなどの調整を行います。
- **より積極的に買い場を探したい場合**: `buy_signal.rsi_oversold` を `35.0` に上げることで、フラグが出やすくなります。
- **反映**: ファイルを保存後、ブラウザでページをリロード（または全件更新）すると新しい基準が適用されます。
...
## next step
- **[改善]銘柄の売却タイミングを表示する注目フラグの追加**: 購入のための注目フラグを可視化する機能は追加したが、売却タイミングを可視化する注目フラグも追加する。
    -これで売買両面のタイミングを視覚的に把握できるようにする。
- **[UI改善]売却タイミングを表示する注目フラグを追加したが、そのデータをフィルタする項目が購入注目などとなっているため、内容を修正する**
    - 現在、フィルタが、購入注目(すべて) 購入チャンス(高勝率のみ) となっているが、売却タイミングも考慮したフィルタ内容に修正する
- **[改善]保有銘柄の過熱感を表す情報の可視化を行う**: 保有銘柄全体での過熱感を表す機能を検討して実装する。
- **[UI改善]背景をダークモードに変える機能の追加**: ダークモード好きな方向けにダークモードを選択して画面を黒くできるようにする。
    - その際に、各種ボタンや色が見えなくならないように調整が必要なので、その点含めたUI設計を考える必要あり。
    - 場合によっては、通常モードとダークモードで色合いを変更する処理などの追加を検討（メンテしにくくなるかどうか？など事前に検討してから判断が必要）
...
- **修正例**:
  ```html
  <!-- v=1.15 を v=1.16 に更新 -->
  <script src="/static/js/analysis.js?v=1.16"></script>
  ```
