...
## next step
- **[改善]株価データの取得方法の抜本的見直しをする**: 取得株数が増えてきた影響もあり、スクレイピングで取得する情報が大きくなることで、サイトでのガードに引っ掛かるようになってしまった。
    - 有料のAPIサービスを利用するか、yfinanceなどのある程度有名なツールを利用してデータを引っ張れるように改良する必要あり。
    - 何かしらの対策を講じた仕組みに変更する。まずは検討から。
- **[改善]注目フラグの再整理_売買判断を単純にできるように**: 現状の注目フラグの内容をどう判断するか？についてより精緻かつ単純に判断できるようにしたい
- **[bug]同一銘柄に注目＋長期調整のシグナルと別に長期調整のシグナルが表示されることがある**: 判定方法が不明瞭になるので調査して改善を行う
- **[UI改善]注目シグナルで強い買いシグナルが出た場合、シグナルの色を強調する**: ゲームのガチャのように1位：虹色、2位：金色、3位：銀色などにできないか？
