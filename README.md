...
- **総合評価スコア**: 複数の指標（PER, PBR, ROE, 利回り, **連続増配**, **トレンド**）を総合的に評価した「スコア」を**最大13点**の星印で表示し、銘柄の魅力を多角的に判断できます。
...
- **資産履歴の自動記録**: 分析ページを開くたびに、その時点の資産状況（評価額・損益・配当など）を月次履歴として自動的にデータベースに保存します。これにより、将来的に資産推移を振り返ることが可能になります。
...
- **対応**: 総合評価スコアの星印を、評価項目に応じて色分けして表示するように改善しました。
  - **ファンダメンタルズ評価**: PER, PBR, ROE, 配当利回り, 連続増配年数に基づく評価を**黄色（★）**で表示。
  - **トレンド評価**: 株価移動平均（25日・75日）に基づく評価を**青色（★）**で表示。
  - これにより、銘柄がファンダメンタルズ面とテクニカル面（トレンド）のどちらで高く評価されているかを一目で判別できるようになりました。
