document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderFilteredRecommendationReport(data) {
        ...
        let cardsHtml = recs.map((item, idx) => {
            ...
            const rawYield = item.dividend_yield_str || (matchedAsset ? (matchedAsset.dividend_yield || matchedAsset.yield) : null);
            ...
            if (!shieldText && matchedAsset) {
                const doe = matchedAsset.doe;
                const consec = matchedAsset.consecutive_increase_years;
            ...
            let contribBadge = '';
            if (matchedAsset) {
                const contrib = matchedAsset.dividend_contribution;
                if (contrib && contrib > 0) {
                    contribBadge = `<span class="rec-mini-badge">📊 配当シェア ${contrib.toFixed(1)}%</span>`;
                } else {
