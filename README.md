...
## next step
- **[改善]アプリの修正をして更新すると再度yahooファイナンスからデータ取得して遅い**: 修正を活発に行なっている時に、何度もデータ取得し直しが発生して待ち時間がかかるのを防ぎたい。
    - 良い案がないか検討。取得した情報のDB化が妥当？
- **[改善]株価の下落などが起きている時の原因（業績悪化など）情報を確認とれるようにする**: 
    - 現在は指標情報と株価の変動から購入サインなどを把握している。
    - 業績悪化や減配などの情報があればその情報を把握できるようにしたい。
    - 手法から検討する
- **[改善]株価データをDB保存することで時系列での分析を行えるようにする**: 過去の株価や配当情報などを把握可能になる
    - 現在はスクレイピングしたデータは1時間だけキャッシュに記録するようにしているが、DBに保存するようにすればキャッシュに頼らなくても良くなる。
    - 注意点として、スクレイピングした時間も記録しておく必要あり。日中に取得したデータだと、当日の終値ではないタイミングもあるため。
      - この辺りの課題を解決するには、平日毎日、後場の終了時間から1時間以上経過してからバッチ処理でデータを取得してテーブル保存する。などの仕組みの方が良さそう。
      - どう保存するのか？目的次第のため、その辺りを議論してから決める。
