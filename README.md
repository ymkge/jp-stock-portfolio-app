...
- **年間配当合計の課税・非課税区分の追加**: 国内株式、外国株式について、税金を考慮した仕組みにする
    - 具体的にはNISAなどの非課税口座の場合、配当金はそのままで良いが、課税口座の場合は受取額から税金を減じた金額を表示可能にする。
    - 税金は社会情勢によって利率が変わる為、外部ファイルで手動で税金計算結果を修正できるようにしておきたい。

  上記の設計方針
   1. 税率設定の外部ファイル化:
       * README.mdの指示通り、税率を外部ファイルで管理します。プロジェクトルートにtax_config.jsonという名称
         で設定ファイルを作成します。
       * このファイルには、資産タイプごと（国内株、米国株など）の税率と、非課税対象となる口座種別を定義しま
         す。これにより、将来の税制変更や新しい資産タイプの追加にも柔軟に対応できます。


   2. 計算ロジックの集約:
       * 配当金の計算は、現在portfolio_manager.py内のcalculate_holding_values関数で行われています。ここに税
         金計算ロジックも集約させ、責務を明確にします。


   3. バックエンドの責務:
       * app.pyはtax_config.jsonを読み込み、計算に必要な情報をportfolio_manager.pyに渡す役割を担います。
       * portfolio_manager.pyは、渡された情報（資産データ、保有情報、税率設定）を基に、税引き前の年間配当と
         、税引き後の年間配当の両方を計算します。


   4. APIレスポンスの拡張:
       * /api/portfolio/analysisエンドポイントが返すJSONに、新たに「税引後年間配当」(estimated_annual_divid
         end_after_tax)と、ポートフォリオ全体の「税引後年間配当合計」(total_annual_dividend_after_tax)を追
         加します。


   5. フロントエンドの表示:
       * 分析ページ(analysis.html)のサマリーと保有銘柄一覧テーブルに、税引き後の配当金額を表示する欄を追加
         します。
       * まずは両方の金額（税引き前・後）を併記する形で実装し、UIの複雑化を避けます。

  設計タスク


   1. `tax_config.json`の作成:
       * 税率と非課税口座を定義するJSONファイルを作成します。
   2. バックエンドの修正 (`app.py`と`portfolio_manager.py`):
       * app.pyでtax_config.jsonを読み込みます。
       * portfolio_manager.pyのcalculate_holding_values関数を修正し、口座種別に応じて税金計算を行うロジック
         を追加します。
   3. APIレスポンスの修正 (`app.py`):
       * get_portfolio_analysis関数を修正し、ポートフォリオ全体の税引き後配当合計を計算し、APIレスポンスに
         含めます。
   4. フロントエンドの修正 (`analysis.html`と`analysis.js`):
       * サマリーとテーブルに「税引後年間配当」の表示領域を追加し、APIから受け取ったデータを表示するようana
         lysis.jsを更新します。
   5. CSVエクスポート機能の更新 (`portfolio_manager.py`):
       * 分析データのCSVエクスポート(create_analysis_csv_data)に「税引後年間配当」の列を追加します。


- **時価総額がN/Aになっている原因の調査と対策**： JST22時ごろに国内株式の時価総額が全てN/Aになっていた。
    - 外国株式は大丈夫で、国内株式のみ。
    - 時間帯によってこうなってるかも知れなしので、後日別の時間帯に確認するところから。
