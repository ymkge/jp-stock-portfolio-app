...
## next step
- **[Bug]分析ページに現在値(外貨)という項目があるが、これは外貨ではない為、表示を現在値のみにする**: ヘッダーの内容の修正
- **[改善]保有銘柄一覧の画面からフィボナッチとRCI(26)の表示は不要の為、非表示にする**: 不要な項目を表示対象外にする
...
- **修正例**:
  ```html
  <!-- v=1.13 を v=1.14 に更新 -->
  <script src="/static/js/analysis.js?v=1.14"></script>
  ```
