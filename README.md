...
- **[改善]株価データの取得方法の抜本的見直しをする**: 取得株数が増えてきた影響もあり、スクレイピングで取得する情報が大きくなることで、yahooファイナンスのサイトでのセキュリティガードに引っ掛かるようになりました。
    - 何かしらの対策を講じた仕組みに変更する必要がある為、まずは検討から始めたいです。（対応中）
- **[改善]通信基盤の最適化（セッション・マネジメント）**: 「1銘柄ごとに回線を切断・再接続する」非効率な方式になっていた為、「1つの安定した回線を賢く使い回す」方式へと変更する
- **[調査]analysisページの銘柄情報が表示されない**: キャッシュがある場合でも、待機時間が発生してしまっていたので、修正する。
