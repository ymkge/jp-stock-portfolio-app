...
## next step
- **保有銘柄サマリーと傾向分析**: [機能]現在の保有銘柄情報と保有株数からどんな傾向のポートフォリオになっているか？を分析して表示する機能を実装する
    - 現在のポートフォリオのリスクなどの情報を把握してリバランスなどの参考になる機能
- **購入注目フラグの実装**: 総合評価とは別軸で、現時点で購入すると以降上がる可能性が高い。という情報を判定して、その対象になった銘柄は注目フラグをUI上で立てる。
    - 総合評価よりも購入タイミング判定可能な指標を追加する。
