...
- **米国株式データ取得の不安定性**:
    - **問題点**: 米国株式の銘柄登録は可能ですが、銘柄名、市場、現在株価、時価総額、PER、配当利回りなどの主要な情報が「N/A」と表示され、正しく取得できていません。前日比のみが部分的に表示されることがあります。このデータ取得不全が原因で、ポートフォリオ分析ページが完全に表示されない状態です。
    - **原因**: 米国株式のデータソースとして利用しているYahoo Finance (US)のウェブサイトのHTML構造が頻繁に変更されており、スクレイピングに使用しているセレクタが現在の構造と一致していません。これにより`USStockScraper`が必要な情報を取得できず、`N/A`が返され、バックエンドの計算処理でエラーが発生しています。
    - **今後の対応**:
        - **セレクタの継続的な更新**: Yahoo Finance (US)のHTML構造を定期的に監視し、`USStockScraper`内のセレクタを迅速に更新する必要があります。
        - **より堅牢なスクレイピング手法の検討**: 可能であれば、国内株式や投資信託で採用している`__PRELOADED_STATE__`のようなページ埋め込みJSONデータの活用を検討します。また、JavaScriptで動的に生成されるコンテンツに対応するため、Seleniumなどのヘッドレスブラウザを利用したスクレイピングの導入も視野に入れます（ただし、環境構築の複雑化が伴います）。
        - **代替データソースの検討**: Yahoo Finance (US)以外の無料または有料の株式データAPIの利用を検討し、スクレイピングへの依存度を低減します。
        - **エラーハンドリングの強化**: `N/A`データや予期せぬ形式のデータが渡された場合でも、アプリケーション全体がクラッシュせず、可能な限り部分的な情報でも表示できるよう、各層でのエラーハンドリングをさらに強化します。
