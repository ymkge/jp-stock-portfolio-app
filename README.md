...
5. 銘柄の「保有管理区分」の追加と、分析ページの追加
  - 銘柄毎に保有区分を操作のボタンで追加管理可能にする。保有区分がついた銘柄には「平均購入単価」と「数量」を入力可能になる。
    - また、「平均購入単価」と「数量」を掛けて「投資額」を算出し、これらの項目と「現在株価」を利用して、「評価額」、「損益」、「損益（％）」、「年間配当金予想」を算出する
      - 「投資額」:「平均購入単価」*「数量」
      - 「評価額」:「現在株価」*「数量」
      - 「損益」： 「評価額」-「投資額」
      - 「損益（％）」:(「投資額」/「評価額」) - 1.0
  ...
  - 上記の情報が入った銘柄に関して、ポートフォリオ分析ページを作成する
    - 保有管理区分がONになっている銘柄のみの専用ページを作成
      - 「銘柄コード」、「銘柄名」、「業種」、「数量」、「取得単価」、「現在株価」、「損益」、「損益（％）」、「評価額」、「年間配当金予想」の一覧ページを作成
        - このページの情報にCSV出力ボタンを追加して、CSVデータを出力可能にする
          - 用途：自分の保有銘柄の一覧を管理可能にする。
      - 「業種」毎に「評価額」を合計し「業種別評価額合計」を算出し「業種」と 「業種別評価額合計」から円グラフで業種別の保有比率情報を可視化する
        - 用途：業種別の保有比率情報が一目でわかる為、偏りがないか？を管理することでリバランスの判断情報にする
※ 検討事項：保有管理区分の付いた情報から他にも分析に使えるネタがないか？を検討必要。
