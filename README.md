...
- [機能追加]国内株式銘柄の指標の追加
    - フィボナッチラインを追加
    - RCIを追加
        - yahooファイナンスで取得できる時系列の株価情報をもとにする。
        - ポートフォリオ一覧にも両方の指標項目を追加して表示する。
        - 総合評価スコアにも移動平均と同じように星の評価を追加
            - 総合評価の星判定はフィボナッチで星１つ、RCIで星１つとする。
- [機能追加]資産推移の可視化（グラフ化）
    - 今回構築した `portfolio_history.db` のデータを活用し、総資産額や配当金の月次推移をグラフで表示するページを作成する。
    - 過去の特定の月のポートフォリオ詳細を確認できる機能を追加する。
...
- **修正例**:
  ```html
  <!-- v=1.7 を v=1.8 に更新 -->
  <script src="/static/js/analysis.js?v=1.8"></script>
  ```
