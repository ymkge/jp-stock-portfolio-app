...
- **[改善]参照先の情報が取得できない場合の表示を修正する**: 現在はネットワークエラーなどと表示されているが、エラーコード:500, 403などが発生しているなどもう少し詳細を表示することで、対策検討しやすい状態にする。
- **[改善]アプリケーション起動後に銘柄情報取得処理のサマリ情報を表示する**: 現在は、銘柄情報の個別の取得ログがサーバ側に表示されている。
    - 処理が終わった時に、何件の銘柄（国内株式/国内投信/米国株式)毎に取得件数、取得までにかかった時間をサーバ上にログ表示する。
        - これによって、どの程度の処理時間がかかっているのか？を正確に把握できるようにする。
