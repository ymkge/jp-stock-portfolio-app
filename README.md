...
- **保有情報管理**: 銘柄ごとに取得単価と数量を記録し、評価額、損益、損益率、年間配当予想を自動で計算・表示します。
- **ポートフォリオ分析**: 保有銘柄のみを対象とした分析ページで、ポートフォリオ全体のサマリーや業種別の保有比率を円グラフで視覚的に確認できます。
...
### 保有管理機能と分析ページの追加 (2025/11/10 対応済み)
- **対応**:
  - **保有管理機能**: 銘柄ごとに取得単価・数量を管理し、評価額・損益・年間配当などを自動計算する機能を追加。
  - **ポートフォリオ分析ページ**: 保有銘柄のみを対象とした分析ページを新設。ポートフォリオ全体のサマリー（総投資額、総評価額など）を表示。
  - **業種別グラフ**: `Chart.js`を導入し、分析ページに業種別の保有比率を円グラフで可視化する機能を追加。
  - **CSVダウンロード**: 分析ページの内容をCSVでダウンロードする機能を追加。
  - **データ構造の移行**: 将来の拡張性のため、データ保存形式 (`portfolio.json`) をオブジェクトのリスト形式に移行。古い形式からの自動変換機能も実装。
