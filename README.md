...
- **サイトアクセス量を減らす為に全件更新の連続ボタン押しの対応の調整**: 現状仕様が曖昧な為、整理して対応方法を決める必要あり。
  - 現状整理:
    - ポートフォリオ一覧を開くと、yahooファイナンスからスクレイピングで株価データを取得する
    - ポートフォリオ分析ページを開いてから、ポートフォリオ一覧に戻って再度、ポートフォリオ分析ページに移動すると、「分析データの取得に失敗しました。(リクエストが多すぎます。あと 0分8秒 お待ちください。)」などと表示される。
    - この表示をされないようにしたい。
    - ただし、毎回yahooファイナンスから同じデータをスクレイピングするとサーバ負荷がかかり、BANされる可能性があるので、一度データを取得したら１時間はキャッシュからデータを取得できるようにする仕様は維持したい。
