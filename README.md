...
### Yahoo!ファイナンスへのアクセス負荷軽減
- **問題点**: 銘柄追加や保有情報更新のたびに、登録されている全銘柄のデータを再取得するため、短時間にリクエストが集中し、アクセス制限のリスクがある。
- **対応方針**: 以下のステップで段階的に改修を進め、アクセス負荷を軽減する。
  1. **Step 1 (バックエンド改善): データキャッシュの導入**
     - サーバー側で一度取得した銘柄データを一定時間（例: 5分）キャッシュする機構を実装する。
     - これにより、短期間に繰り返しリクエストが発生しても、Yahoo!ファイナンスへのアクセスを大幅に削減する。
  2. **Step 2 (フロントエンド改善): 差分更新の実装**
     - 銘柄の追加・削除・更新時に、ページ全体を再読み込みするのではなく、変更があった箇所だけをJavaScriptで動的に書き換えるようにする。
  3. **Step 3 (最終形): ハイブリッド更新と手動更新機能**
     - 上記2つの仕組みを組み合わせ、UIに「全件更新」ボタンを追加する。通常操作は差分更新で軽くし、ユーザーが必要な時だけ全データを強制的にリフレッシュできるようにする。

### その他の課題/改善事項
1. 銘柄毎にチェックボックスで選択可能な機能の追加[実装済み]
