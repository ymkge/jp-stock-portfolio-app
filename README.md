...
### 投資信託（インデックスファンド等）への対応

**目的:**
現在の国内個別株中心のポートフォリオ管理に加え、「eMAXIS Slim 全世界株式（オール・カントリー）」に代表されるようなインデックスファンド（投資信託）も統合的に管理できるようにする。

**調査結果 (2025/11/23):**
- Yahoo!ファイナンスの投資信託ページ (`https://finance.yahoo.co.jp/quote/0331418A`) を調査した結果、個別株ページで利用した埋め込みJSON (`__PRELOADED_STATE__`) は存在しないことが判明。
- しかし、ページのHTMLソースを直接解析することで、**基準価額、純資産総額、信託報酬**などの主要な情報を抽出できる見込みが立った。
- このHTML解析手法は、将来的なサイトデザインの変更に影響を受ける可能性がある点に留意が必要。

**対応方針:**
性質の異なる資産を無理に統合せず、新しい**「資産タイプ (`asset_type`)」**の概念を導入し、ロジックを分離して実装する方針とする。このアプローチにより、コードの保守性を高めつつ、将来的には「米国株」や「仮想通貨」などへのさらなる拡張も容易になる。また、異なる資産タイプの評価額や損益を合算し、ポートフォリオ全体のサマリーを表示することも可能になる。

**実装ステップ:**
1.  **`scraper.py`の改修:**
    - `BeautifulSoup4`ライブラリを導入する。
    - 投資信託ページのHTMLを解析し、基準価額や信託報酬などを取得する新しい関数 `fetch_fund_data()` を作成する。
2.  **データモデルの移行:**
    - `portfolio.json` の各資産に `asset_type` キー（例: `"jp_stock"`, `"investment_trust"`）を追加する。
    - アプリケーション起動時に、既存のデータ（`asset_type`がないデータ）を新しい形式に移行する処理を実装する。
3.  **バックエンド (`app.py`) の改修:**
    - 資産を追加・取得するAPIで `asset_type` を考慮し、呼び出すスクレイパー関数 (`fetch_stock_data` / `fetch_fund_data`) を分岐させる。
4.  **フロントエンドの改修:**
    - UI上で国内株式と投資信託を区別して表示できるようにする。（例: タブ切り替え、テーブルカラムの最適化など）
...
現状の課題点と今後の対応方針
  ...
  これまでの進捗概要:
  国内株式と投資信託の両方を管理できるポートフォリオアプリケーションの基本的な機能（データモデル、API、フロ
  ントエンドUI、タブ切り替え、分析ページ、CSVダウンロード）は実装済みです。国内株式が表示されない問題や、投
  資信託追加後のタブ自動切り替え機能も修正・改善しました。

  現在の主要な課題:

   1. 投資信託のデータ取得（スクレイピング）が不完全:
       * 現象: SyntaxErrorが解決し、アプリケーションが起動したとしても、fetch_fund_data関数がYahoo!ファイナン
         スから基準価額、前日比、純資産総額、信託報酬などの情報を正しく取得できていない可能性があります。これ
         までのテスト（python3 scraper.pyの実行結果）では、ファンド名以外はN/Aとなっていました。ファンド名に
         も「の基準価額・投資信託情報」という余計な文字列が付いていました。
       * 原因: Yahoo!ファイナンスのHTML構造が頻繁に変更されるため、scraper.py内のCSSセレクタが追いついていな
         いことが原因です。User-AgentやAccept-Languageヘッダーの強化、メインページと履歴ページからの情報取得
         の試みは行いましたが、セレクタの再修正が必要です。

  次回開発再開時に行うべきこと:

   1. fetch_fund_dataがまだN/Aを返す場合、ユーザーの環境で表示されているYahoo!ファイナンスの投資信託ページのHT
      ML構造を詳細に確認し、scraper.py内のセレクタを再修正する。
       * ファンド名: h1タグから余計な文字列を除去するロジックが必要になる可能性が高い。
       * 信託報酬: main_soup.find('dt', string='信託報酬')
         が機能しない場合、dtタグのテキスト内容が完全に一致しないか、別のタグに存在するかを確認する。
       * 基準価額、前日差、純資産総額: history_soup.select_one('tbody tr')
         が機能しない場合、履歴ページのテーブル構造が変更されている可能性がある。
